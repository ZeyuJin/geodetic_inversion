function [zgrid,xgrid,ygrid] = xyz2surface(x,y,z,w,xnodes,ynodes,smoothparam_in,varargin)
% xyz2surface: estimates a surface on a 2d grid, based on scattered data
...
if (params.tilesize < max(nx,ny))
  ...
  %A = [W*A;Areg*(smoothparam*NA/NR)];
    A = [W*A;Areg*(smoothparam_in*NA/NR)];

  %rhs = [rhs;zeros(nreg,1)];
  rhs = [w.*rhs;zeros(nreg,1)];
  % do we have a mask to apply?
  if params.maskflag
