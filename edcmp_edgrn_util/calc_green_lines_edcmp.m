function G=calc_green_lines_edcmp(slip_model,data_insar,finp)
...
% for InSAR data
yrec=data_insar(:,1);
xrec=data_insar(:,2);%commented out for test
% yrec=data_insar(:,2);
% xrec=data_insar(:,1);
ve=data_insar(:,4);
vn=data_insar(:,5);
...
for ipatch = 1:npatch
   ...
   % slip_patch=slip_model(ipatch,8);  % scaled by the moment  commented
   % out by xiaoyu for testing.

   xs_patch=slip_model(ipatch,5);
   ys_patch=slip_model(ipatch,4);%commented out by xiaoyu
   % xs_patch=slip_model(ipatch,4);
   % ys_patch=slip_model(ipatch,5);
...
%    contributions from the strike-slip components
    ...
    %disp(size(ssu1));%for xiaoyu's test
      for nn=1:nps
...
%  contributions from the clvd component
      ...
      Un(k) = sum(Ur.*co-Ut.*si);
      Ue(k) = sum(Ur.*si+Ut.*co);
      Up(k) = -sum(Uz);
   end
