function G=calc_green_azo_edcmp(slip_model,data_insar,finp);
...
vn=data_insar(:,5);
% vz=data_insar(:,6);
theta_az = mod(90 + atan2d(vn, ve), 360);
...
%    G(:,ipatch) = Ue.*ve + Un.*vn + Up.*vz;
   G(:,ipatch) = cosd(theta_az).*Ue + sind(theta_az).*Un;
...
%     G(:,ipatch+npatch) = Ue.*ve + Un.*vn + Up.*vz;
   G(:,ipatch+npatch) = cosd(theta_az).*Ue + sind(theta_az).*Un;
